...
public:
    ...
    /// Add line for line rendering 'MESH_LINES'
    int AddLine(glm::vec3 pointA, glm::vec3 pointB, Color color);
    
    /// Add a plain sub mesh to the vertex buffer.
    int AddPlain(float x, float y, float z, float width, float height, Color color, float uCoord=1, float vCoord=1, float uStart=0, float vStart=0, unsigned int uOffset=0, unsigned int vOffset=0);
    ...
    /// Add a cube sub mesh to the vertex buffer.
    void AddCube(float x, float y, float z, float width, float height, float depth, Color color);
    ...
    void AddSphere(float x, float y, float z, float radius, unsigned int rings, unsigned int sectors, Color color, bool invertWinding);
    
    /// Add a cone sub mesh to the vertex buffer.
    void AddCone(float x, float y, float z, float radius, float height, unsigned int sectors, Color color);
    ...
    /// Trigger this mesh to be uploaded to GPU memory.
    bool doLoadBuffer;
    
    /// Trigger this mesh to be removed from GPU memory.
    bool doUnloadBuffer;
