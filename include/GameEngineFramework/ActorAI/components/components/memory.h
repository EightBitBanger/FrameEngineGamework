...
public:
    ...
    /// Scale the emotional state matrix by a single value.
    void DegradeEmotionalState(float amount);
    
    /// Scale the emotional state matrix by an emotion embedding.
    void DegradeEmotionalState(const EmotionalEmbedding& embedding, float threshold);
