...
void ActorSystem::UpdateAnimationState(Actor* actor) {
    ...
    for (unsigned int a = 0; a < actor->genetics.mGeneticRenderers.size(); a++) {
        ...
        case ActorState::Animation::LimbHolding:
            UpdateAnimationInHand(geneRenderer->transform.matrix, actor, a);
            break;
        }
...
void ActorSystem::UpdateHeadRotation(glm::mat4& matrix, Actor* actor, unsigned int a) {
    ...
    float targetYaw = glm::degrees(glm::atan(xx, zz)) + 180.0f;
    if (!actor->state.mIsFacing) 
        targetYaw += 180.0f;
        
    targetYaw = WrapDeg360(targetYaw);
    
    float bodyYaw = WrapDeg360(actor->navigation.mRotation.y);
    float deltaYaw = ShortestDeltaDeg(bodyYaw, targetYaw);
    
    const float maxLookAngle = 94.0f; 
    deltaYaw = glm::clamp(deltaYaw, -maxLookAngle, maxLookAngle);
    
    actor->navigation.mLookAt.y = WrapDeg360(bodyYaw + deltaYaw);
    
    // Rotate head to target look at point
...
void ActorSystem::ApplyAnimationRotation(glm::mat4& matrix, Actor* actor, unsigned int a) {
    ...
    float animationLength = glm::length(actor->animation.mAnimation[a]);
    matrix = glm::rotate(matrix, glm::radians(animationLength), 
                         glm::normalize(glm::vec3(actor->animation.mAnimation[a])));
}
...
void ActorSystem::HandleAnimationSwing(Actor* actor, unsigned int a, glm::vec4& animationFactor, float animationMaxSwingRange, bool animationDirection) {
    ...
    if (doInverseAnimation) {
        actor->animation.mAnimation[a] += animationFactor;
        if (glm::any(glm::greaterThan(actor->animation.mAnimation[a], glm::vec4(animationMaxSwingRange)))) 
            actor->animation.mAnimation[a].w = actor->genetics.mGenes[a].doInverseAnimation ? 1 : -1;
    } else {
        actor->animation.mAnimation[a] -= animationFactor;
        if (glm::any(glm::lessThan(actor->animation.mAnimation[a], glm::vec4(-animationMaxSwingRange)))) 
            actor->animation.mAnimation[a].w = actor->genetics.mGenes[a].doInverseAnimation ? -1 : 1;
    }
}

void ActorSystem::EnsureNonZeroAnimationState(Actor* actor, unsigned int a) {
    if (actor->animation.mAnimation[a].x == 0.0f) 
        actor->animation.mAnimation[a].x += 0.0001f;
    if (actor->animation.mAnimation[a].y == 0.0f) 
        actor->animation.mAnimation[a].y += 0.0001f;
