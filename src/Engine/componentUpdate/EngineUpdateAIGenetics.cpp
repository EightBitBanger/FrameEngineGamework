...
void EngineSystemManager::UpdateActorGenetics(unsigned int index) {
    ...
    for (unsigned int a = 0; a < numberOfGenes; a++) {
        ...
        Transform transform;
        orientation = glm::vec4(transform.rotation.w, transform.rotation.x, transform.rotation.y, transform.rotation.z);
...
void EngineSystemManager::UpdateActorGenetics(unsigned int index) {
    ...
    for (unsigned int a=0; a < numberOfGenes; a++) {
        ...
        // Bounding box
        newRenderer->SetBoundingBoxMax(glm::vec3(3, 3, 3));
        newRenderer->SetBoundingBoxMin(glm::vec3(-3, -3, -3));
