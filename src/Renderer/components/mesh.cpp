...
int Mesh::AddLine(glm::vec3 pointA, glm::vec3 pointB, Color color) {
    Vertex vertex[2];
    
    vertex[0] = Vertex(pointA.x, pointA.y, pointA.z, color.r, color.g, color.b,  0, 1, 0,  0, 0);
    vertex[1] = Vertex(pointB.x, pointB.y, pointB.z, color.r, color.g, color.b,  0, 1, 0,  0, 0);
    ...
    // We pass 0, 0, 0 as the offset because pointA and pointB are already in world/local space coordinates
    return AddSubMesh(0.0f, 0.0f, 0.0f, subBuffer.vertexBuffer, subBuffer.indexBuffer, false);
}

int Mesh::AddPlain(float x, float y, float z, float width, float height, Color color, float uCoord, float vCoord, float uStart, float vStart, unsigned int uOffset, unsigned int vOffset) {
    Vertex vertex[4];
...
void Mesh::AddCube(float x, float y, float z, float width, float height, float depth, Color color) {
    ...
    auto pushQuadIndices = [&](unsigned int baseVertex) {
        // Using standard Counter-Clockwise (CCW) winding
    ...
    // +Z (Front)  normal (0, 0, 1)
    {
        ...
        vertices.push_back(Vertex(-halfWidth, +halfHeight, +halfDepth, color.r, color.g, color.b,  0, 0, 1,  0, 0)); // Top-Left
        vertices.push_back(Vertex(-halfWidth, -halfHeight, +halfDepth, color.r, color.g, color.b,  0, 0, 1,  0, 1)); // Bottom-Left
        vertices.push_back(Vertex(+halfWidth, -halfHeight, +halfDepth, color.r, color.g, color.b,  0, 0, 1,  1, 1)); // Bottom-Right
        vertices.push_back(Vertex(+halfWidth, +halfHeight, +halfDepth, color.r, color.g, color.b,  0, 0, 1,  1, 0)); // Top-Right
    ...
    {
        unsigned int baseVertex = static_cast<unsigned int>(vertices.size());
        vertices.push_back(Vertex(+halfWidth, +halfHeight, +halfDepth, color.r, color.g, color.b,  1, 0, 0,  0, 0)); // Top-Left
        vertices.push_back(Vertex(+halfWidth, -halfHeight, +halfDepth, color.r, color.g, color.b,  1, 0, 0,  0, 1)); // Bottom-Left
        vertices.push_back(Vertex(+halfWidth, -halfHeight, -halfDepth, color.r, color.g, color.b,  1, 0, 0,  1, 1)); // Bottom-Right
        vertices.push_back(Vertex(+halfWidth, +halfHeight, -halfDepth, color.r, color.g, color.b,  1, 0, 0,  1, 0)); // Top-Right
        pushQuadIndices(baseVertex);
    }
    ...
    {
        unsigned int baseVertex = static_cast<unsigned int>(vertices.size());
        vertices.push_back(Vertex(-halfWidth, +halfHeight, -halfDepth, color.r, color.g, color.b,  0, 1, 0,  0, 0)); // Top-Left
        vertices.push_back(Vertex(-halfWidth, +halfHeight, +halfDepth, color.r, color.g, color.b,  0, 1, 0,  0, 1)); // Bottom-Left
        vertices.push_back(Vertex(+halfWidth, +halfHeight, +halfDepth, color.r, color.g, color.b,  0, 1, 0,  1, 1)); // Bottom-Right
        vertices.push_back(Vertex(+halfWidth, +halfHeight, -halfDepth, color.r, color.g, color.b,  0, 1, 0,  1, 0)); // Top-Right
        pushQuadIndices(baseVertex);
    }
...
void Mesh::AddSphere(float x, float y, float z, float radius, unsigned int rings, unsigned int sectors, Color color, bool invertWinding) {
    std::vector<Vertex> vertices;
    std::vector<Index> indices;
    ...
    for (r = 0; r < rings - 1; r++) {
        for (s = 0; s < sectors - 1; s++) {
            // Get the 4 corners of the current quad
            unsigned int current_row_current_sector = r * sectors + s;
            unsigned int current_row_next_sector    = r * sectors + (s + 1);
            unsigned int next_row_next_sector       = (r + 1) * sectors + (s + 1);
            unsigned int next_row_current_sector    = (r + 1) * sectors + s;
            
            if (invertWinding) {
                // Original order (inward facing)
                indices.push_back(current_row_current_sector);
                indices.push_back(current_row_next_sector);
                indices.push_back(next_row_next_sector);
                
                indices.push_back(current_row_current_sector);
                indices.push_back(next_row_next_sector);
                indices.push_back(next_row_current_sector);
            } else {
                // Flipped order (outward facing)
                indices.push_back(current_row_current_sector);
                indices.push_back(next_row_next_sector);
                indices.push_back(current_row_next_sector);
...
void Mesh::AddCone(float x, float y, float z, float radius, float height, unsigned int sectors, Color color) {
    std::vector<Vertex> vertices;
    std::vector<Index>  indices;
    
    // A cone needs at least 3 sides to form a 3D volume
    if (sectors < 3) sectors = 3; 
    
    // We need 1 vertex for the center of the base, 1 for the tip, and 'sectors' for the ring
    vertices.reserve(sectors + 2);
    indices.reserve(sectors * 6);
    
    float halfHeight = height * 0.5f;
    
    // Index 0: Base center (Normal pointing strictly down: 0, -1, 0)
    vertices.push_back(Vertex(0.0f, -halfHeight, 0.0f, color.r, color.g, color.b, 0, -1, 0, 0.5f, 0.5f));
    
    // Index 1: Tip of the cone (Normal pointing strictly up: 0, 1, 0)
    vertices.push_back(Vertex(0.0f, +halfHeight, 0.0f, color.r, color.g, color.b, 0,  1, 0, 0.5f, 0.5f));
    
    // Calculate the upward tilt of the side normals.
    // A cone's side normal points outward and slightly up, perpendicular to the slope.
    float ny = radius / height; 
    
    // Generate the base ring vertices
    for (unsigned int i = 0; i < sectors; ++i) {
        float theta = 2.0f * glm::pi<float>() * static_cast<float>(i) / static_cast<float>(sectors);
        float cx = std::cos(theta);
        float cz = std::sin(theta);
        
        // Un-normalized normal direction
        float nx = cx;
        float nz = cz;
        ...
        // Basic circular UV mapping for the base ring
        float u = (cx + 1.0f) * 0.5f;
        float v = (cz + 1.0f) * 0.5f;
        
        vertices.push_back(Vertex(cx * radius, -halfHeight, cz * radius, color.r, color.g, color.b, nx, ny, nz, u, v));
    }
    
    // Generate indices with CCW Winding Order
    for (unsigned int i = 0; i < sectors; ++i) {
        // Vertex indices for the current slice of the ring
        unsigned int current = 2 + i;
        unsigned int next    = 2 + ((i + 1) % sectors);
        
        // Base Triangle: Center -> Current -> Next (Visible from the bottom)
        Index b0; b0.index = 0;       indices.push_back(b0);
        Index b1; b1.index = current; indices.push_back(b1);
        Index b2; b2.index = next;    indices.push_back(b2);
        
        // Side Triangle: Tip -> Next -> Current (Visible from the outside)
        Index s0; s0.index = 1;       indices.push_back(s0);
        Index s1; s1.index = next;    indices.push_back(s1);
        Index s2; s2.index = current; indices.push_back(s2);
    }
