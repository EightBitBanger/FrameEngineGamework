...
void UserInterfaceSystem::UpdateButtonElements() {
    for (unsigned int i = 0; i < mButtonElements.Size(); i++) {
        Button* button = mButtonElements[i];
        if (!button->isActive || button->callback == nullptr) 
            continue;
        
        int mouseRelativeX = Input.mouseX - Platform.windowArea.x - 7;
        int mouseRelativeY = Input.mouseY - Platform.windowArea.y - 30;
        
        
        if ((button->doLeftClick && Input.CheckMouseLeftPressed()) || 
            (button->doCenterClick && Input.CheckMouseMiddlePressed()) || 
            (button->doRightClick && Input.CheckMouseRightPressed())) {
            
            if (mouseRelativeX > button->x && 
                mouseRelativeX < button->x + button->w && 
                mouseRelativeY > button->y && 
                mouseRelativeY < button->y + button->h) {
                button->callback();
            }
        }
    }
...
Panel* UserInterfaceSystem::CreatePanel(void) {
    Panel* panel = mPanelElements.Create();
    panel->name = Int.ToString(uniqueID) + "_panel";
    uniqueID++;
    return panel;
}
...
Sprite* UserInterfaceSystem::CreateSprite(void) {
    Sprite* sprite = mSpriteElements.Create();
    sprite->name = Int.ToString(uniqueID) + "_sprite";
    uniqueID++;
    return sprite;
}
...
Button* UserInterfaceSystem::CreateButton( void(*callback)() ) {
    Button* button = mButtonElements.Create();
    button->sprite = mSpriteElements.Create();
    button->callback = callback;
    return button;
}

bool UserInterfaceSystem::DestroyButton(Button* buttonPtr) {
