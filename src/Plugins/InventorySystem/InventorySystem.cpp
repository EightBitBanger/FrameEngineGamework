...
bool InventorySystem::AddItem(std::string name) {
    ...
    if (mItems[mSelectorIndex] == name || mItems[mSelectorIndex] == "") {
        if (mCount[mSelectorIndex] < itemClass.stackMax) {
            ...
            mSlotSprites[mSelectorIndex]->texture->UploadTextureToGPU(matTag->buffer, (unsigned int)matTag->width, (unsigned int)matTag->height, 4, GL_NEAREST, GL_NEAREST);
    ...
    for (unsigned int i=0; i < mItems.size(); i++) {
        ...
        // Load item image
        TextureTag* matTag = Resources.FindTextureTag(name);
        mSlotSprites[i]->texture->UploadTextureToGPU(matTag->buffer, matTag->width, matTag->height, 4,  GL_NEAREST, GL_NEAREST);
