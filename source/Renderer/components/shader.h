...
public:
    ...
    void SetModelMatrix(glm::mat4 &ModelMatrix) {glUniformMatrix4fv(modelMatrixLocation, 1, 0, &ModelMatrix[0][0]);}
    void SetProjectionMatrix(glm::mat4 &projectionMatrix) {glUniformMatrix4fv(projectionMatrixLocation, 1, 0, &projectionMatrix[0][0]);}
    
    void SetMaterialColor(Color color) {glUniform4f(materialColorLocation, color.r, color.g, color.b, color.a);}
    
    void SetTextureSampler(unsigned int index) {glUniform1i(samplerLocation, index);}
