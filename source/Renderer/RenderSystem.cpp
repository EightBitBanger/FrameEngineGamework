...
void RenderSystem::RenderFrame(float deltaTime) {
    ...
    glm::vec3 lookingAngle = glm::vec3(1);
    ...
    for (std::vector<Scene*>::iterator it = mRenderQueue.begin(); it != mRenderQueue.end(); ++it) {
        ...
        // Get the camera assigned to this scene
        ...
        currentCamera->forward.x = cos( (currentCamera->transform.orientation.x * 180) / PI );
        currentCamera->forward.y = tan( (currentCamera->transform.orientation.y * 180) / PI );
        currentCamera->forward.z = sin( (currentCamera->transform.orientation.x * 180) / PI );
        ...
        view = glm::lookAt(eye, lookingAngle, currentCamera->up);
